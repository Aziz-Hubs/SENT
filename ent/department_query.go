...
func (_q *DepartmentQuery) QueryMembers() *EmployeeQuery {
...
func (_q *DepartmentQuery) QueryHead() *EmployeeQuery {
	query := (&EmployeeClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *DepartmentQuery) WithMembers(opts ...func(*EmployeeQuery)) *DepartmentQuery {
	query := (&EmployeeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DepartmentQuery) WithHead(opts ...func(*EmployeeQuery)) *DepartmentQuery {
	query := (&EmployeeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := _q.withMembers; query != nil {
		if err := _q.loadMembers(ctx, query, nodes,
			func(n *Department) { n.Edges.Members = []*Employee{} },
			func(n *Department, e *Employee) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *DepartmentQuery) loadMembers(ctx context.Context, query *EmployeeQuery, nodes []*Department, init func(*Department), assign func(*Department, *Employee)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *DepartmentQuery) loadHead(ctx context.Context, query *EmployeeQuery, nodes []*Department, init func(*Department), assign func(*Department, *Employee)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Department)
	for i := range nodes {
		if nodes[i].department_head == nil {
			continue
		}
		fk := *nodes[i].department_head
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "department_head" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}

...
func (_q *TransactionQuery) QueryTenant() *TenantQuery {
...
func (_q *TransactionQuery) QueryLedgerEntries() *LedgerEntryQuery {
...
func (_q *TransactionQuery) QueryJournalEntries() *JournalEntryQuery {
...
func (_q *TransactionQuery) QueryRecording() *RecordingQuery {
...
func (_q *TransactionQuery) QueryApprovedBy() *UserQuery {
...
func (_q *TransactionQuery) WithTenant(opts ...func(*TenantQuery)) *TransactionQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
func (_q *TransactionQuery) WithLedgerEntries(opts ...func(*LedgerEntryQuery)) *TransactionQuery {
	query := (&LedgerEntryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TransactionQuery) WithJournalEntries(opts ...func(*JournalEntryQuery)) *TransactionQuery {
	query := (&JournalEntryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TransactionQuery) WithRecording(opts ...func(*RecordingQuery)) *TransactionQuery {
	query := (&RecordingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TransactionQuery) WithApprovedBy(opts ...func(*UserQuery)) *TransactionQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if _q.withTenant != nil || _q.withApprovedBy != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withLedgerEntries; query != nil {
		if err := _q.loadLedgerEntries(ctx, query, nodes,
			func(n *Transaction) { n.Edges.LedgerEntries = []*LedgerEntry{} },
			func(n *Transaction, e *LedgerEntry) { n.Edges.LedgerEntries = append(n.Edges.LedgerEntries, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withJournalEntries; query != nil {
		if err := _q.loadJournalEntries(ctx, query, nodes,
			func(n *Transaction) { n.Edges.JournalEntries = []*JournalEntry{} },
			func(n *Transaction, e *JournalEntry) { n.Edges.JournalEntries = append(n.Edges.JournalEntries, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withRecording; query != nil {
		if err := _q.loadRecording(ctx, query, nodes, nil,
			func(n *Transaction, e *Recording) { n.Edges.Recording = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withApprovedBy; query != nil {
		if err := _q.loadApprovedBy(ctx, query, nodes, nil,
			func(n *Transaction, e *User) { n.Edges.ApprovedBy = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TransactionQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Tenant)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].tenant_transactions == nil {
			continue
		}
		fk := *nodes[i].tenant_transactions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tenant_transactions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TransactionQuery) loadLedgerEntries(ctx context.Context, query *LedgerEntryQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *LedgerEntry)) error {
	...
	query.Where(predicate.LedgerEntry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(transaction.LedgerEntriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.transaction_ledger_entries
		if fk == nil {
			return fmt.Errorf(`foreign-key "transaction_ledger_entries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "transaction_ledger_entries" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *TransactionQuery) loadJournalEntries(ctx context.Context, query *JournalEntryQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *JournalEntry)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Transaction)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.JournalEntry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(transaction.JournalEntriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.transaction_journal_entries
		if fk == nil {
			return fmt.Errorf(`foreign-key "transaction_journal_entries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "transaction_journal_entries" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *TransactionQuery) loadRecording(ctx context.Context, query *RecordingQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Recording)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].RecordingID == nil {
			continue
		}
...
func (_q *TransactionQuery) loadApprovedBy(ctx context.Context, query *UserQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].transaction_approved_by == nil {
			continue
		}
		fk := *nodes[i].transaction_approved_by
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "transaction_approved_by" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TransactionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withRecording != nil {
			_spec.Node.AddColumnOnce(transaction.FieldRecordingID)
		}
	}

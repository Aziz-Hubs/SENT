...
func (_q *ProductQuery) QueryTenant() *TenantQuery {
...
func (_q *ProductQuery) QueryMovements() *StockMovementQuery {
...
func (_q *ProductQuery) QueryReservations() *InventoryReservationQuery {
...
func (_q *ProductQuery) QueryVendor() *AccountQuery {
...
func (_q *ProductQuery) WithTenant(opts ...func(*TenantQuery)) *ProductQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
func (_q *ProductQuery) WithMovements(opts ...func(*StockMovementQuery)) *ProductQuery {
	query := (&StockMovementClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProductQuery) WithReservations(opts ...func(*InventoryReservationQuery)) *ProductQuery {
	query := (&InventoryReservationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProductQuery) WithVendor(opts ...func(*AccountQuery)) *ProductQuery {
	query := (&AccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := _q.withMovements; query != nil {
		if err := _q.loadMovements(ctx, query, nodes,
			func(n *Product) { n.Edges.Movements = []*StockMovement{} },
			func(n *Product, e *StockMovement) { n.Edges.Movements = append(n.Edges.Movements, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withReservations; query != nil {
		if err := _q.loadReservations(ctx, query, nodes,
			func(n *Product) { n.Edges.Reservations = []*InventoryReservation{} },
			func(n *Product, e *InventoryReservation) { n.Edges.Reservations = append(n.Edges.Reservations, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ProductQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Product, init func(*Product), assign func(*Product, *Tenant)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Product)
	for i := range nodes {
		if nodes[i].tenant_products == nil {
			continue
		}
		fk := *nodes[i].tenant_products
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tenant_products" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ProductQuery) loadMovements(ctx context.Context, query *StockMovementQuery, nodes []*Product, init func(*Product), assign func(*Product, *StockMovement)) error {
	...
	query.Where(predicate.StockMovement(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.MovementsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.product_movements
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_movements" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_movements" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *ProductQuery) loadReservations(ctx context.Context, query *InventoryReservationQuery, nodes []*Product, init func(*Product), assign func(*Product, *InventoryReservation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *ProductQuery) loadVendor(ctx context.Context, query *AccountQuery, nodes []*Product, init func(*Product), assign func(*Product, *Account)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Product)
	for i := range nodes {
		if nodes[i].product_vendor == nil {
			continue
		}
		fk := *nodes[i].product_vendor
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "product_vendor" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}

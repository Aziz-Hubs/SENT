...
func (_q *AccountQuery) QueryJournalEntries() *JournalEntryQuery {
...
func (_q *AccountQuery) QueryRecurringInvoices() *RecurringInvoiceQuery {
...
func (_q *AccountQuery) WithJournalEntries(opts ...func(*JournalEntryQuery)) *AccountQuery {
	query := (&JournalEntryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) WithRecurringInvoices(opts ...func(*RecurringInvoiceQuery)) *AccountQuery {
	query := (&RecurringInvoiceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withJournalEntries; query != nil {
		if err := _q.loadJournalEntries(ctx, query, nodes,
			func(n *Account) { n.Edges.JournalEntries = []*JournalEntry{} },
			func(n *Account, e *JournalEntry) { n.Edges.JournalEntries = append(n.Edges.JournalEntries, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withRecurringInvoices; query != nil {
		if err := _q.loadRecurringInvoices(ctx, query, nodes,
			func(n *Account) { n.Edges.RecurringInvoices = []*RecurringInvoice{} },
			func(n *Account, e *RecurringInvoice) {
				n.Edges.RecurringInvoices = append(n.Edges.RecurringInvoices, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *AccountQuery) loadJournalEntries(ctx context.Context, query *JournalEntryQuery, nodes []*Account, init func(*Account), assign func(*Account, *JournalEntry)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.JournalEntry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.JournalEntriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.account_journal_entries
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_journal_entries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_journal_entries" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *AccountQuery) loadRecurringInvoices(ctx context.Context, query *RecurringInvoiceQuery, nodes []*Account, init func(*Account), assign func(*Account, *RecurringInvoice)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.RecurringInvoice(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.RecurringInvoicesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.account_recurring_invoices
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_recurring_invoices" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_recurring_invoices" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

...
func (_q *CredentialQuery) QueryTenant() *TenantQuery {
...
func (_q *CredentialQuery) QueryAsset() *AssetQuery {
...
func (_q *CredentialQuery) QueryOneTimeLinks() *OneTimeLinkQuery {
...
func (_q *CredentialQuery) WithTenant(opts ...func(*TenantQuery)) *CredentialQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
func (_q *CredentialQuery) WithAsset(opts ...func(*AssetQuery)) *CredentialQuery {
	query := (&AssetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CredentialQuery) WithOneTimeLinks(opts ...func(*OneTimeLinkQuery)) *CredentialQuery {
	query := (&OneTimeLinkClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CredentialQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Credential, error) {
	...
	if _q.withTenant != nil || _q.withAsset != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withTenant; query != nil {
		if err := _q.loadTenant(ctx, query, nodes, nil,
			func(n *Credential, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withOneTimeLinks; query != nil {
		if err := _q.loadOneTimeLinks(ctx, query, nodes,
			func(n *Credential) { n.Edges.OneTimeLinks = []*OneTimeLink{} },
			func(n *Credential, e *OneTimeLink) { n.Edges.OneTimeLinks = append(n.Edges.OneTimeLinks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CredentialQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Credential, init func(*Credential), assign func(*Credential, *Tenant)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Credential)
	for i := range nodes {
		if nodes[i].tenant_credentials == nil {
			continue
		}
		fk := *nodes[i].tenant_credentials
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tenant_credentials" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *CredentialQuery) loadAsset(ctx context.Context, query *AssetQuery, nodes []*Credential, init func(*Credential), assign func(*Credential, *Asset)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Credential)
	for i := range nodes {
...
func (_q *CredentialQuery) loadOneTimeLinks(ctx context.Context, query *OneTimeLinkQuery, nodes []*Credential, init func(*Credential), assign func(*Credential, *OneTimeLink)) error {
	...
	query.Where(predicate.OneTimeLink(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(credential.OneTimeLinksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.credential_one_time_links
		if fk == nil {
			return fmt.Errorf(`foreign-key "credential_one_time_links" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "credential_one_time_links" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

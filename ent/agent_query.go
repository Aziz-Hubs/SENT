...
func (_q *AgentQuery) QueryTenant() *TenantQuery {
...
func (_q *AgentQuery) WithTenant(opts ...func(*TenantQuery)) *AgentQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
func (_q *AgentQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Agent, init func(*Agent), assign func(*Agent, *Tenant)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Agent)
	for i := range nodes {
		if nodes[i].tenant_agents == nil {
			continue
		}
		fk := *nodes[i].tenant_agents
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tenant_agents" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}

...
func (_q *LedgerEntryQuery) QueryTransaction() *TransactionQuery {
...
func (_q *LedgerEntryQuery) QueryTenant() *TenantQuery {
...
func (_q *LedgerEntryQuery) WithTransaction(opts ...func(*TransactionQuery)) *LedgerEntryQuery {
	query := (&TransactionClient{config: _q.config}).Query()
...
func (_q *LedgerEntryQuery) WithTenant(opts ...func(*TenantQuery)) *LedgerEntryQuery {
	query := (&TenantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(ledgerentry.FieldAmount).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.LedgerEntry.Query().
//		Select(ledgerentry.FieldAmount).
//		Scan(ctx, &v)
...
func (_q *LedgerEntryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*LedgerEntry, error) {
	...
	if _q.withTransaction != nil || _q.withAccount != nil || _q.withTenant != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withTransaction; query != nil {
		if err := _q.loadTransaction(ctx, query, nodes, nil,
			func(n *LedgerEntry, e *Transaction) { n.Edges.Transaction = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withTenant; query != nil {
		if err := _q.loadTenant(ctx, query, nodes, nil,
			func(n *LedgerEntry, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *LedgerEntryQuery) loadTransaction(ctx context.Context, query *TransactionQuery, nodes []*LedgerEntry, init func(*LedgerEntry), assign func(*LedgerEntry, *Transaction)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*LedgerEntry)
	for i := range nodes {
		if nodes[i].transaction_ledger_entries == nil {
			continue
		}
		fk := *nodes[i].transaction_ledger_entries
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "transaction_ledger_entries" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *LedgerEntryQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*LedgerEntry, init func(*LedgerEntry), assign func(*LedgerEntry, *Tenant)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*LedgerEntry)
	for i := range nodes {
		if nodes[i].tenant_ledger_entries == nil {
			continue
		}
		fk := *nodes[i].tenant_ledger_entries
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tenant_ledger_entries" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}

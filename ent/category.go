...
func (e CategoryEdges) TenantOrErr() (*Tenant, error) {
	if e.Tenant != nil {
		return e.Tenant, nil
...
func (*Category) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case category.FieldName, category.FieldDescription, category.FieldType:
			values[i] = new(sql.NullString)
		case category.ForeignKeys[0]: // tenant_categories
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case category.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field tenant_categories", value)
			} else if value.Valid {
				_m.tenant_categories = new(int)
				*_m.tenant_categories = int(value.Int64)
			}
		default:
...
// QueryTenant queries the "tenant" edge of the Category entity.
func (_m *Category) QueryTenant() *TenantQuery {
	return NewCategoryClient(_m.config).QueryTenant(_m)
}

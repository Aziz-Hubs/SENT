...
func (_q *ContactQuery) QueryTenant() *TenantQuery {
...
func (_q *ContactQuery) WithTenant(opts ...func(*TenantQuery)) *ContactQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
func (_q *ContactQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Contact, init func(*Contact), assign func(*Contact, *Tenant)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Contact)
	for i := range nodes {
		if nodes[i].tenant_contacts == nil {
			continue
		}
		fk := *nodes[i].tenant_contacts
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tenant_contacts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}

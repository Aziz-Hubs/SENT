...
func (_q *ScriptQuery) QueryTenant() *TenantQuery {
...
func (_q *ScriptQuery) QueryJobs() *JobQuery {
...
func (_q *ScriptQuery) WithTenant(opts ...func(*TenantQuery)) *ScriptQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
func (_q *ScriptQuery) WithJobs(opts ...func(*JobQuery)) *ScriptQuery {
	query := (&JobClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ScriptQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Script, error) {
	...
	if query := _q.withTenant; query != nil {
		if err := _q.loadTenant(ctx, query, nodes, nil,
			func(n *Script, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withJobs; query != nil {
		if err := _q.loadJobs(ctx, query, nodes,
			func(n *Script) { n.Edges.Jobs = []*Job{} },
			func(n *Script, e *Job) { n.Edges.Jobs = append(n.Edges.Jobs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ScriptQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Script, init func(*Script), assign func(*Script, *Tenant)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Script)
	for i := range nodes {
		if nodes[i].tenant_scripts == nil {
			continue
		}
		fk := *nodes[i].tenant_scripts
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tenant_scripts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ScriptQuery) loadJobs(ctx context.Context, query *JobQuery, nodes []*Script, init func(*Script), assign func(*Script, *Job)) error {
	...
	query.Where(predicate.Job(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(script.JobsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.script_jobs
		if fk == nil {
			return fmt.Errorf(`foreign-key "script_jobs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "script_jobs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

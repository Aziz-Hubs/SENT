...
type ApplicationQuery struct {
	...
	withJobPosting *JobPostingQuery
...
// QueryJobPosting chains the current query on the "job_posting" edge.
func (_q *ApplicationQuery) QueryJobPosting() *JobPostingQuery {
...
func (_q *ApplicationQuery) QueryCandidate() *CandidateQuery {
...
func (_q *ApplicationQuery) QueryInterviews() *InterviewQuery {
...
func (_q *ApplicationQuery) Clone() *ApplicationQuery {
	...
	return &ApplicationQuery{
		...
		withJobPosting: _q.withJobPosting.Clone(),
...
// WithJobPosting tells the query-builder to eager-load the nodes that are connected to
// the "job_posting" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ApplicationQuery) WithJobPosting(opts ...func(*JobPostingQuery)) *ApplicationQuery {
	query := (&JobPostingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withJobPosting = query
	return _q
}
...
func (_q *ApplicationQuery) WithCandidate(opts ...func(*CandidateQuery)) *ApplicationQuery {
	query := (&CandidateClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ApplicationQuery) WithInterviews(opts ...func(*InterviewQuery)) *ApplicationQuery {
	query := (&InterviewClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(application.FieldStatus).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Application.Query().
//		Select(application.FieldStatus).
//		Scan(ctx, &v)
...
func (_q *ApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Application, error) {
	var (
		...
		loadedTypes = [4]bool{
			_q.withTenant != nil,
			_q.withJobPosting != nil,
			_q.withCandidate != nil,
			_q.withInterviews != nil,
		}
	)
	if _q.withTenant != nil || _q.withJobPosting != nil || _q.withCandidate != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withJobPosting; query != nil {
		if err := _q.loadJobPosting(ctx, query, nodes, nil,
			func(n *Application, e *JobPosting) { n.Edges.JobPosting = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCandidate; query != nil {
		if err := _q.loadCandidate(ctx, query, nodes, nil,
			func(n *Application, e *Candidate) { n.Edges.Candidate = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *ApplicationQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Application, init func(*Application), assign func(*Application, *Tenant)) error {
	...
	for i := range nodes {
		if nodes[i].tenant_applications == nil {
			continue
		}
		fk := *nodes[i].tenant_applications
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tenant_applications" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ApplicationQuery) loadJobPosting(ctx context.Context, query *JobPostingQuery, nodes []*Application, init func(*Application), assign func(*Application, *JobPosting)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Application)
	for i := range nodes {
		if nodes[i].job_posting_applications == nil {
			continue
		}
		fk := *nodes[i].job_posting_applications
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(jobposting.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "job_posting_applications" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ApplicationQuery) loadCandidate(ctx context.Context, query *CandidateQuery, nodes []*Application, init func(*Application), assign func(*Application, *Candidate)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Application)
	for i := range nodes {
		if nodes[i].candidate_applications == nil {
			continue
		}
		fk := *nodes[i].candidate_applications
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "candidate_applications" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ApplicationQuery) loadInterviews(ctx context.Context, query *InterviewQuery, nodes []*Application, init func(*Application), assign func(*Application, *Interview)) error {
	...
	for _, n := range neighbors {
		fk := n.application_interviews
		if fk == nil {
			return fmt.Errorf(`foreign-key "application_interviews" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "application_interviews" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

...
func (_q *TicketQuery) QueryAsset() *AssetQuery {
...
// QueryRemediationSteps chains the current query on the "remediation_steps" edge.
func (_q *TicketQuery) QueryRemediationSteps() *RemediationStepQuery {
	query := (&RemediationStepClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TicketQuery) WithAsset(opts ...func(*AssetQuery)) *TicketQuery {
	query := (&AssetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithRemediationSteps tells the query-builder to eager-load the nodes that are connected to
// the "remediation_steps" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TicketQuery) WithRemediationSteps(opts ...func(*RemediationStepQuery)) *TicketQuery {
	query := (&RemediationStepClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRemediationSteps = query
	return _q
}
...
func (_q *TicketQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ticket, error) {
	...
	if _q.withTenant != nil || _q.withRequester != nil || _q.withAssignee != nil || _q.withAsset != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withAsset; query != nil {
		if err := _q.loadAsset(ctx, query, nodes, nil,
			func(n *Ticket, e *Asset) { n.Edges.Asset = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withRemediationSteps; query != nil {
		if err := _q.loadRemediationSteps(ctx, query, nodes,
			func(n *Ticket) { n.Edges.RemediationSteps = []*RemediationStep{} },
			func(n *Ticket, e *RemediationStep) { n.Edges.RemediationSteps = append(n.Edges.RemediationSteps, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TicketQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Tenant)) error {
	...
	for i := range nodes {
		if nodes[i].tenant_tickets == nil {
			continue
		}
		fk := *nodes[i].tenant_tickets
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tenant_tickets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TicketQuery) loadRequester(ctx context.Context, query *UserQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *User)) error {
	...
	nodeids := make(map[int][]*Ticket)
	for i := range nodes {
		if nodes[i].user_requested_tickets == nil {
			continue
		}
		fk := *nodes[i].user_requested_tickets
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_requested_tickets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TicketQuery) loadAssignee(ctx context.Context, query *UserQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *User)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_assigned_tickets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TicketQuery) loadAsset(ctx context.Context, query *AssetQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Asset)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Ticket)
	for i := range nodes {
		if nodes[i].asset_tickets == nil {
			continue
		}
		fk := *nodes[i].asset_tickets
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "asset_tickets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TicketQuery) loadRemediationSteps(ctx context.Context, query *RemediationStepQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *RemediationStep)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Ticket)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.RemediationStep(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(ticket.RemediationStepsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.ticket_remediation_steps
		if fk == nil {
			return fmt.Errorf(`foreign-key "ticket_remediation_steps" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "ticket_remediation_steps" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

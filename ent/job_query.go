...
type JobQuery struct {
	...
	withExecutions *JobExecutionQuery
	withFKs        bool
...
func (_q *JobQuery) QueryTenant() *TenantQuery {
...
func (_q *JobQuery) QueryScript() *ScriptQuery {
...
func (_q *JobQuery) QueryExecutions() *JobExecutionQuery {
...
func (_q *JobQuery) WithTenant(opts ...func(*TenantQuery)) *JobQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
func (_q *JobQuery) WithScript(opts ...func(*ScriptQuery)) *JobQuery {
	query := (&ScriptClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *JobQuery) WithExecutions(opts ...func(*JobExecutionQuery)) *JobQuery {
	query := (&JobExecutionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *JobQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Job, error) {
	...
	if _q.withTenant != nil || _q.withScript != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withTenant; query != nil {
		if err := _q.loadTenant(ctx, query, nodes, nil,
			func(n *Job, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withScript; query != nil {
		if err := _q.loadScript(ctx, query, nodes, nil,
			func(n *Job, e *Script) { n.Edges.Script = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withExecutions; query != nil {
		if err := _q.loadExecutions(ctx, query, nodes,
			func(n *Job) { n.Edges.Executions = []*JobExecution{} },
			func(n *Job, e *JobExecution) { n.Edges.Executions = append(n.Edges.Executions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *JobQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Job, init func(*Job), assign func(*Job, *Tenant)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Job)
	for i := range nodes {
		if nodes[i].tenant_jobs == nil {
			continue
		}
		fk := *nodes[i].tenant_jobs
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tenant_jobs" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *JobQuery) loadScript(ctx context.Context, query *ScriptQuery, nodes []*Job, init func(*Job), assign func(*Job, *Script)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Job)
	for i := range nodes {
		if nodes[i].script_jobs == nil {
			continue
		}
		fk := *nodes[i].script_jobs
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "script_jobs" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *JobQuery) loadExecutions(ctx context.Context, query *JobExecutionQuery, nodes []*Job, init func(*Job), assign func(*Job, *JobExecution)) error {
	...
	query.Where(predicate.JobExecution(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(job.ExecutionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.job_executions
		if fk == nil {
			return fmt.Errorf(`foreign-key "job_executions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "job_executions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *JobQuery) Modify(modifiers ...func(s *sql.Selector)) *JobSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *JobSelect) Modify(modifiers ...func(s *sql.Selector)) *JobSelect {

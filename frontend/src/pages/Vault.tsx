...
import { Folder, FileText, Download, Trash2, FolderPlus, Upload, ChevronRight, Home, Eye, History, Loader2, FilePlus, Archive, HardDrive } from "lucide-react"
...
export function Vault() {
  ...
  const handleCreateTextFile = async () => {
      ...
      try {
        // @ts-ignore
        await window.go.vault.VaultBridge.SaveFile(path, b64, 1) // Default tenant 1
        fetchFiles(currentPath)
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
        ...
        try {
            // @ts-ignore
            await window.go.vault.VaultBridge.SaveFile(path, base64, 1) // Default tenant 1
            fetchFiles(currentPath)
      ...
      <Card className="border-none shadow-md overflow-hidden">
            ...
            <div className="flex items-center space-x-2 text-xs text-muted-foreground font-mono bg-background/50 w-fit px-3 py-1 rounded-full border">
                ...
                {currentPath.split('/').filter(Boolean).map((part, i) => (
                    <div key={i} className="flex items-center">
                        ...
                        <span className="font-bold text-blue-500">{part}</span>
                    </div>
                ))}
            </div>
        </CardHeader>
        <CardContent className="p-0">
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={5} className="text-center py-20"><Loader2 className="h-10 w-10 animate-spin mx-auto text-blue-500" /></TableCell></TableRow>
              ) : files.length === 0 ? (
                  ...
                  <TableRow key={file.path} className="group hover:bg-muted/30 transition-colors cursor-pointer" onClick={() => file.isDir && handleNavigate(file.path)}>
                    ...
                    <TableCell className="text-right pr-6">
                        <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                            {!file.isDir && (
                                <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-500 hover:bg-blue-100" onClick={(e) => { e.stopPropagation(); handlePreview(file); }}>
                                    <Eye className="h-4 w-4" />
                                </Button>
                            )}
                            {!file.isDir && (
                                <Button variant="ghost" size="icon" className="h-8 w-8 text-green-500 hover:bg-green-100" onClick={(e) => { e.stopPropagation(); handleFileDownload(file); }}>
                                    <Download className="h-4 w-4" />
                                </Button>
                            )}
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-amber-500 hover:bg-amber-100" onClick={(e) => { e.stopPropagation(); }}>
                                <History className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-red-500 hover:bg-red-100" onClick={(e) => { e.stopPropagation(); handleDelete(file.path); }}>
        ...
        <DialogContent className="max-w-4xl h-[85vh] flex flex-col bg-background border-none shadow-2xl p-0 overflow-hidden">
            ...
            <div className="flex-1 overflow-hidden bg-muted/10">
                {previewFile?.name.toLowerCase().endsWith('.pdf') ? (
                    ...
                    <pre className="p-8 w-full h-full overflow-auto font-mono text-xs whitespace-pre-wrap text-foreground">
                        {atob(previewContent || "")}
                    </pre>
                ) : (
            ...
            <DialogFooter className="p-4 bg-muted/30 border-t">
                ...
                {previewFile && (
                    <Button className="bg-blue-600 hover:bg-blue-700 gap-2 text-white font-black uppercase tracking-widest h-11 px-6 shadow-lg shadow-blue-500/20" onClick={() => handleFileDownload(previewFile)}>
